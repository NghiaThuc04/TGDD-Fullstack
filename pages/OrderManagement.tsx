...
import { getAllOrdersApi, updateOrderStatusApi, deleteOrderApi, updateOrderApi } from '../services/api';
            ...
            <div className="p-6 md:p-10 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 flex justify-between items-center shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setSelectedOrder(null)}
                  className="w-12 h-12 bg-white rounded-2xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-red-500 hover:rotate-90 transition-all shadow-sm text-xl font-black"
                >×</button>
              </div>
            </div>
                  ...
                  <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm space-y-3">
                    ...
                    {selectedOrder.status === 'pending' && selectedOrder.paymentMethod === 'Transfer' && (
                      <button disabled={!!processingId} onClick={() => handleUpdateStatus(selectedOrder.id, 'paid')}
                        className="w-full bg-blue-600 text-white py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-100 disabled:opacity-50"
                    ...
                    {(selectedOrder.status === 'paid' || (selectedOrder.status === 'pending' && selectedOrder.paymentMethod === 'COD')) && (
                      <button disabled={!!processingId} onClick={() => handleUpdateStatus(selectedOrder.id, 'shipping')}
                        className="w-full bg-purple-600 text-white py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-purple-700 transition-all flex items-center justify-center gap-2 shadow-lg disabled:opacity-50"
                    ...
                    {selectedOrder.status === 'shipping' && (
                      <button disabled={!!processingId} onClick={() => handleUpdateStatus(selectedOrder.id, 'completed')}
                        className="w-full bg-green-600 text-white py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-green-700 transition-all flex items-center justify-center gap-2 shadow-lg disabled:opacity-50"
                    ...
                    {['pending', 'paid'].includes(selectedOrder.status) && (
                      <button disabled={!!processingId} onClick={() => handleUpdateStatus(selectedOrder.id, 'cancelled')}
                        className="w-full bg-orange-50 text-orange-500 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-orange-500 hover:text-white transition-all disabled:opacity-50"
                      >❌ Hủy đơn hàng</button>
                    )}
                  </div>
